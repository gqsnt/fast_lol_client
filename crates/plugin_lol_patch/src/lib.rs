...
impl IsApiRequest for PostLolPatchV1ProductsLeagueOfLegendsStopPatchingRequest {
    const METHOD: Method = Method::POST;
    type ReturnType = Value;
    fn get_url(&self) -> String {"/lol-patch/v1/products/league_of_legends/stop-patching-request".to_string()}
...
impl IsApiRequest for PutLolPatchV1GamePatchUrl {
    ...
    type ReturnType = Value;
    fn get_url(&self) -> String {"/lol-patch/v1/game-patch-url".to_string()}
    fn get_body(&self) -> Option<Value> {
        Some(to_value(&self.body).unwrap())
    }
}
...
impl IsApiRequest for PutLolPatchV1Ux {
    const METHOD: Method = Method::PUT;
    type ReturnType = Value;
    fn get_url(&self) -> String {"/lol-patch/v1/ux".to_string()}
    fn get_body(&self) -> Option<Value> {
        Some(to_value(&self.body).unwrap())
    }
}
